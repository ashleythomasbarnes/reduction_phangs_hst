...
def get_anchoring_offset(hdu1, hdu2, hdu3, hdu_stars, filter='', rootdir='./', appdir='hst_contsub/', make_plots=True):
    ...
    x_per = np.percentile(data1, [0.1, 75])
    y_per = np.percentile(data2, [0.1, 75])
    ...
    if make_plots: 
        ...
        #data
        ax1.scatter(data1, data2, c='k', alpha=0.01, s=1, rasterized=True)
        ...
        # ax1.set_xlim([offset-std, offset+std])
        # ax1.set_ylim([-std, std])

        ax1.set_xlim(np.nanpercentile(data1, [0.01,99.99]))
        ax1.set_ylim(np.nanpercentile(data2, [0.01,99.99]))
        ...
        #data
        ax2.scatter(data1, data2, c='k', alpha=0.01, s=1, rasterized=True)

        #bins 
        ax2.scatter(bin_values[0], bin_values[1], fc='none', ec='C0', alpha=1, s=30, zorder=5)
        ax2.plot(bin_values[0], bin_values[1], c='C0', alpha=1, zorder=5)

        # fits 
        offset = (0 - (intercept_bins)) / slope_bins
        ax2.plot(x_fit, y_fit_bins, color='C0', linewidth=2, linestyle=':', label=f'y = {slope_bins:.4f}x + {intercept_bins:.4g}')
        ax2.plot(x_fit, x_fit, 'k', linewidth=2, linestyle=':', label=f'y = x')
        ax2.plot([offset, offset], [-100,0], color='C0', linewidth=2, linestyle=':', label=f'Offset = {offset:.4g}')

        ax2.set_xlabel('Flux density (MUSE) [erg/s/cm-2/A/pix]')
        ax2.set_ylabel('Flux density (HST smoothed, regrid) [erg/s/cm-2/A/pix]')
        ax2.legend(title=filter, loc='upper left')
        ax2.grid(True, ls=':', color='k', alpha=0.2)
