...
def save_fittables_offsets(fit_f555w, fit_f65Xn, fit_f814w, rootdir='./', appdir='hst_contsub/'):
    ...
    table_fits.write(rootdir+appdir+'/table_fits_bgcorr.fits', overwrite=True)
    table_fits.write(rootdir+appdir+'/table_fits_bgcorr.csv', overwrite=True)


def save_fittables_slope(fit_halpha, rootdir='./', appdir='hst_contsub/'):
    
    table_fits = fit_halpha
    table_fits.write(rootdir+appdir+'/table_fits_NII.fits', overwrite=True)
    table_fits.write(rootdir+appdir+'/table_fits_NII.csv', overwrite=True)
